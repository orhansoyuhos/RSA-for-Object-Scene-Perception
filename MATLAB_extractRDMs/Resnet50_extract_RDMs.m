...
if strcmp(dataset, 'Imagenet')
    imagenet_resnet50 = load('..\Python_extractFeatures\resultDic\RSA_resultDic_imagenet_resnet50.mat');
    results_resnet50 = imagenet_resnet50;
elseif strcmp(dataset, 'Places365')
    places365_resnet50 = load('..\Python_extractFeatures\resultDic\RSA_resultDic_places365_resnet50.mat');
    results_resnet50 = places365_resnet50;
end

%% Compute RDM for deepnn
N_layer = resnet50_layers;

% make features flat for each sample
resnet50Layer_samples = cell(1,N_layer);
fns = fieldnames(results_resnet50);

for ii = 3:N_layer+2
    tmp_array = results_resnet50.(fns{ii});
    resnet50Layer_samples{ii-2} = reshape(tmp_array, n_val, []);
end
...
for i = 1:N_layer
    ...
    tmp_later = resnet50Layer_samples{i};
